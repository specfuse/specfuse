...
- **Event log entries** appended to `/events/INIT-YYYY-NNNN.jsonl` for state transitions the role owns: `initiative_created` (on initiative intake), `feature_state_changed` (on `drafting → validating` and `validating → planning`), and `human_escalation` (on escalations). Every event is piped through `scripts/validate-event.py` and must exit `0` before the append; the `source_version` field is produced by `scripts/read-agent-version.sh specs` at emission time, never eye-cached from `version.md`. See `verify-before-report.md` §3 for the full construction discipline (including the canonical `--file /tmp/event.json` validate-event.py invocation, the JSONL single-line requirement, the canonical safe append pattern, and the timestamp discipline — all absorbed in WU 3.11).
